...
export async function mostrarAdmin() {
    ...
    // Validación única por tu correo
    const { data: { user }, error: userError } = await supabase.auth.getUser();
    if (!user || user.email !== "manirojas207") {
        ...
        : `<ul>
            ${estudiantes.map(est => `
                <li style="margin-bottom:8px;">
                    ...
                    <button data-id="${est.id}" class="borrar-estudiante" style="margin-left:8px;">Eliminar</button>
                </li>
            `).join("")}
        </ul>`
    }`;
        ...
        : `<ul>
            ${actividades.map(act => {
                ...
                const imagenHtml = act.imagen
                    ? `<div style="margin-top:6px;"><img src="${escapeAttr(act.imagen)}" alt="${escapeAttr(act.titulo)}" style="max-width:160px;max-height:120px;object-fit:cover;"></div>`
                    : "";
                return `
                    <li style="margin-bottom:12px;border-bottom:1px solid #eee;padding-bottom:8px;">
                        ...
                        <div style="margin-top:6px;">
                            Nota: <input type="number" min="0" max="5" step="0.1" value="${act.nota ?? ""}" data-id="${act.id}" class="nota-input" style="width:70px;">
                        </div>
                    </li>
                `;
            }).join("")}
    ...
    document.querySelectorAll(".borrar-estudiante").forEach(btn => {
        btn.addEventListener("click", async (e) => {
            ...
            const { error: delError } = await supabase.from("estudiantes").delete().eq("id", id);
            if (delError) {
    ...
    if (guardarBtn) {
        guardarBtn.addEventListener("click", async () => {
            ...
            for (const input of inputs) {
                ...
                if (isNaN(nota)) { errores++; continue; }
